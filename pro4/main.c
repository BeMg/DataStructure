...
int item_cmp(Date* a, Date* b, int n) {
    switch (n) {
    case 0:
        return a->Id - b->Id;
        break;
...
int partition(void *a,int len,int pivot_idx,size_t size,int (*cmp)( Date* a, Date* b)) {
    int i;
...
void quick_sort(void *a,int len,size_t size,int (*cmp)( Date* a, Date* b)) {
...
void bubble_sort(void *a,int len,size_t size,int (*cmp)( Date* a, Date* b)) {
    for(int i=0; i<len; i++) {
...
int stable_cmp(Date* a, Date* b) {
    if(item_cmp(a,b,First) == 0 && item_cmp(a,b,Second) == 0)
        return a->Id - b->Id;
    return 0;
}

void stable_sort(void *a,int len,size_t size) {
    for(int i=0; i<len; i++) {
        for(int j=0; j<len-i-1; j++) {
            if(stable_cmp(a+j*size,a+(j+1)*size)>0)
                swap(a+j*size,a+(j+1)*size,size);
        }
    }
}
...
void Double_sort() {
    if(First!=-1 && abs(CheckForSorted[First])==1)
        quick_sort(data,data_cnt,sizeof(data[0]),Firstcmp);
    else if(First!=-1 && abs(CheckForSorted[First])==2)
        bubble_sort(data,data_cnt,sizeof(data[0]),Firstcmp);
    if(Second!=-1 && abs(CheckForSorted[Second])==1)
        quick_sort(data,data_cnt,sizeof(data[0]),Secondcmp);
    else if(Second!=-1 && abs(CheckForSorted[Second])==2)
        bubble_sort(data,data_cnt,sizeof(data[0]),Secondcmp);
    stable_sort(data,data_cnt,sizeof(data[0]));
}
